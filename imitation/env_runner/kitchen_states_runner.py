...
class KitchenStatesRunner(BaseLowdimRunner):
    def __init__(self,
                 output_dir,
                 dataset_dir,
                 n_train=10,
        ...
        task_fps = fps
        steps_per_render = int(task_fps / fps)

        def env_fcn():
            return MultiStepWrapper(
                VideoRecordingWrapper(
                    gymnasium.make('FrankaKitchen-v1',
...
def run(self, policy: BaseLowdimPolicy):
        ...
        for chunk_idx in range(n_chunks):
            ...
            pbar = tqdm.tqdm(total=self.max_steps, desc=f"Eval KitchenStates {chunk_idx+1}/{n_chunks}", 
