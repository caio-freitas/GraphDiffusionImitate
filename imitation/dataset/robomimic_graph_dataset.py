...
class RobomimicGraphDataset(InMemoryDataset):
    ...
    def _get_node_feats(self, data, idx):
        ...
        if self.mode == "end-effector":
            node_feats = torch.tensor(data["robot0_eef_pos"][idx - 1:idx][0])
            ...
            if self.mode == "task-space":
                ...
                node_feats = torch.tensor(data["robot0_joint_pos"][idx - 1:idx][0])
        ...
        # create tensor of same dimension return super()._get_node_feats(data, idx) as node_feats
